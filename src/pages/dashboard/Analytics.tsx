...
const Analytics = () => {
  ...
  const overTimeData = useMemo(() => {
    ...
    stats.registrations.forEach(r => {
      const week = format(startOfWeek(parseISO(r.created_at)), "dd/MM");
  ...
  const statCards = [
    { label: "Total de inscrições", value: stats?.total ?? 0, icon: Users },
    { label: "Eventos ativos", value: stats?.activeEvents ?? 0, icon: CalendarDays },
    { label: "Média por evento", value: stats?.events?.length ? Math.round((stats?.total ?? 0) / stats.events.length) : 0, icon: TrendingUp },
  ];

  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-display font-bold">Análises</h1>
        <p className="text-muted-foreground">Acompanhe o desempenho dos seus eventos e métricas de inscrição.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <div className="bg-card rounded-xl p-5 sm:p-6">
          <h3 className="font-display font-semibold mb-4">Inscrições por evento</h3>
          {perEventData.length > 0 ? (
            ...
            <p className="text-muted-foreground text-center py-12 text-sm">Sem dados de inscrição ainda.</p>
          )}
        </div>

        <div className="bg-card rounded-xl p-5 sm:p-6">
          <h3 className="font-display font-semibold mb-4">Inscrições ao longo do tempo</h3>
