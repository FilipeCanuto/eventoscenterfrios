        ...
        <div className="space-y-0.5">
          <div className="flex items-center gap-2">
            <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">
              {event.event_date ? format(new Date(event.event_date), "EEE, d MMM", { locale: ptBR }) : "Sem data"}
            </p>
            {!isUpcoming && (
              <Badge className={`${statusColors[event.status] || "bg-muted text-muted-foreground"} border-0 text-[10px]`}>
          ...
          {!isUpcoming && (
            <div className="flex flex-col gap-0.5 text-xs text-muted-foreground pt-1">
              <span className="flex items-center gap-1"><Users className="w-3 h-3" />{count} participantes</span>
    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Crie e gerencie suas páginas de inscrição para eventos.</p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
        <div className="relative max-w-sm flex-1">
          ...
          <Input placeholder="Buscar eventos…" className="pl-10 rounded-full" value={search} onChange={e => setSearch(e.target.value)} />
        </div>
      ...
      {isLoading ? (
                  ...
                  <div className="flex-1 flex flex-col justify-center gap-1.5 py-1">
                    ...
                    <div className="flex gap-2 pt-2">
                      <Button variant="outline" size="sm" className="text-xs rounded-full" asChild onClick={(e) => e.stopPropagation()}>
                        <Link to={`/register/${event.slug}`}><ExternalLink className="w-3 h-3 mr-1" />Ver página</Link>
                      </Button>
                      <Button variant="outline" size="sm" className="text-xs rounded-full" asChild onClick={(e) => e.stopPropagation()}>
                        <Link to={`/dashboard/events/${event.id}`}>Gerenciar</Link>
                      </Button>
                    </div>
                  </div>
                </div>
        ...
        <div className="text-center py-20">
          ...
          <Button asChild>
            <Link to="/dashboard/events/create"><Plus className="w-4 h-4 mr-2" /> Criar evento</Link>
          </Button>
        </div>
      )}
    </div>
