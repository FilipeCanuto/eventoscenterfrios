    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center space-y-4">
          <h1 className="text-4xl font-bold text-foreground">Empresa não encontrada</h1>
          <p className="text-muted-foreground">Esta página de empresa não existe ou ainda não foi configurada.</p>
    ...
    <div className="min-h-screen bg-background">
      ...
      {upcomingEvents.length > 0 && (
        <section className="max-w-6xl mx-auto px-4 sm:px-6 mt-10">
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-2xl font-display font-bold text-foreground">Próximos Eventos</h2>
      ...
      {pastEvents.length > 0 && (
        <section className="max-w-6xl mx-auto px-4 sm:px-6 mt-12 pb-16">
          <h2 className="text-2xl font-display font-bold text-foreground mb-5">Eventos Anteriores</h2>
          {viewMode === "grid" ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
      ...
      {(!events || events.length === 0) && (
        <div className="max-w-6xl mx-auto px-6 py-20 text-center">
          ...
          <h3 className="text-lg font-semibold text-foreground mb-2">Nenhum evento ainda</h3>
          <p className="text-muted-foreground">Volte em breve para ver os próximos eventos.</p>
        </div>
      )}
    </div>
  );
    ...
    <Link to={`/register/${event.slug}`}>
      <Card className="overflow-hidden group hover:shadow-xl transition-all hover:-translate-y-1 border-border/50 h-full">
        ...
        <CardContent className="p-5 space-y-3">
          ...
          <Button className="w-full mt-1 bg-primary" size="sm">Inscrever-se <ArrowRight className="ml-1 w-3.5 h-3.5" /></Button>
        </CardContent>
      </Card>
    </Link>
  );
          ...
          <CardContent className="flex-1 p-5 flex flex-col justify-between gap-3">
            ...
            <div className="flex items-center justify-between">
              <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
                ...
                {regCount !== undefined && (
                  <span className="flex items-center gap-1"><Users className="w-3.5 h-3.5" />{regCount} participantes</span>
                )}
              </div>
              <Button className="bg-primary" size="sm">Inscrever-se <ArrowRight className="ml-1 w-3.5 h-3.5" /></Button>
            </div>
          </CardContent>
        </div>
      </Card>
